...
def set_euid():
    """
    Set settings.NAZS_RUN_AS_USER effective UID for the current process
    ...
    if settings.NAZS_RUN_AS_USER is None:
        logger.info("Not changing EUID, NAZS_RUN_AS_USER is None")
        return

    uid = int(pwd.getpwnam(settings.NAZS_RUN_AS_USER).pw_uid)
    if current != uid:
        try:
            os.seteuid(uid)
            logger.info("Set EUID to %s (%s)" %
                        (settings.NAZS_RUN_AS_USER, os.geteuid()))
        except:
            current_user = pwd.getpwuid(os.getuid()).pw_name
            logger.error("Failed to set '%s' EUID, running as '%s'" %
                         (settings.NAZS_RUN_AS_USER, current_user))
    else:
...
def drop_privileges():
    """
    Set settings.NAZS_RUN_AS_USER UID for the current process
